/**
 * Modified Server-side NFT Retrieval Diagnostic Script
 *
 * This script is a modified version of server-retrieval-diagnostic.js
 * with the wallet address pre-filled from the logs.
 *
 * Usage:
 * Run with: node run-server-diagnostic.js
 */
...
// Configuration
const WALLET_ADDRESS = "0x7411e7942f4c8271d4e636429f374997fdaede17"; // From the logs
const TOKEN_IDS = [1, 2]; // The token IDs we're interested in
const MONGODB_URI = "mongodb://localhost:27017/sosh"; // Update with your MongoDB connection string if needed
    ...
    for (let saleId = 1; saleId <= saleIdTracker; saleId++) {
      try {
        ...
        if (TOKEN_IDS.includes(parseInt(sale.tokenId))) {
          ...
          console.log(`- Start Time: ${formatTimestamp(Number(sale.startTime))} (${sale.startTime})`);
          console.log(`- End Time: ${formatTimestamp(Number(sale.endTime))} (${sale.endTime})`);
          console.log(`- Is Expired: ${isExpired(Number(sale.endTime))}`);
