...
function PostCard({ data, linkable = true, isLoggedInProfile, onDelete }) {
  ...
  const fetchSaleData = useCallback(async (tokenId) => {
    ...
    try {
      ...
      // Fix API URL duplication - remove the /api/V1 prefix as it's already in the baseURL
      const response = await axiosWithAuth.get(`marketplace/sale-info/${tokenId}`);
  ...
  const verifyNftRetrievalEligibility = useCallback(async (tokenId, userAddress) => {
    ...
    try {
      ...
      // Fix API URL duplication - remove the /api/V1 prefix as it's already in the baseURL
      const response = await axiosWithAuth.get(`marketplace/verify-nft/${tokenId}?address=${userAddress}`);
      const requestDuration = Date.now() - requestStartTime;
