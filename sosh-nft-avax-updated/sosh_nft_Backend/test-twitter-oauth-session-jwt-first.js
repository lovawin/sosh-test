/**
 * Twitter OAuth Session Diagnostic Script - JWT First
 * 
...
// Configure session middleware AFTER JWT with memory store for testing
app.use(
  session({
...
app.get('/manual-request-token', async (req, res) => {
  try {
    console.log('\n=== MANUAL REQUEST TOKEN (JWT FIRST) ===');
...
app.get('/auth/twitter', (req, res, next) => {
  console.log('\n=== PASSPORT TWITTER LOGIN (JWT FIRST) ===');
...
app.get('/auth/twitter/callback', 
  (req, res, next) => {
    console.log('\n=== PASSPORT TWITTER CALLBACK (JWT FIRST) ===');
    console.log('Query parameters:', req.query);
    console.log('Session before authentication:', req.session);
    console.log('Session ID before authentication:', req.sessionID);
    console.log('Cookies:', req.cookies);
...
app.get('/', (req, res) => {
  res.send(`
    <h1>Twitter OAuth Session Diagnostic (JWT First)</h1>
...
const PORT = 3004;
app.listen(PORT, () => {
  console.log(`Diagnostic server (JWT First) running on http://localhost:${PORT}`);
});
