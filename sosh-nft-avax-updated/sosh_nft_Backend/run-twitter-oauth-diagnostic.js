 ...
 * 
 * This script will:
 * 1. Start both diagnostic servers (session-first and JWT-first configurations)
...
const sessionFirstServer = runDiagnosticServer(
  path.join(__dirname, 'test-twitter-oauth-session.js'),
  path.join(logsDir, 'session-first.log')
);

const jwtFirstServer = runDiagnosticServer(
  path.join(__dirname, 'test-twitter-oauth-session-jwt-first.js'),
  path.join(logsDir, 'jwt-first.log')
);

// Print instructions
console.log('\n=== TWITTER OAUTH DIAGNOSTIC INSTRUCTIONS ===\n');
console.log('Two diagnostic servers are now running:');
console.log('1. Session First: http://localhost:3003');
console.log('2. JWT First: http://localhost:3004');
...
console.log(`- Session First: ${path.join(logsDir, 'session-first.log')}`);
console.log(`- JWT First: ${path.join(logsDir, 'jwt-first.log')}`);
console.log('\nPress Ctrl+C to stop both servers when testing is complete.');

// Handle process termination
process.on('SIGINT', () => {
  console.log('\nStopping diagnostic servers...');
  sessionFirstServer.kill();
  jwtFirstServer.kill();
  process.exit(0);
});
